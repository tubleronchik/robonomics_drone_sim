...
class Px4Controller:
    ...
    def payment(self):
        ...
        program = f'{config.dir()}robonomics io read launch'
        robot_adreddres = config.addresses()['drone']
        employer_addres = config.addresses()['employer']
        wrong_addres = config.addresses()['not_drone']
        process = subprocess.Popen(program, shell=True, stdout=subprocess.PIPE)

        while True:
            try:
                output = process.stdout.readline()
                if str(output.strip()) == f"b'{employer_addres} >> {robot_adreddres} : true'":
                    print('Job paid')
                    return True
                return False
    ...
    def start(self):
        ...
        if self.payment():
            ...
            print('Not my job has been paid!')
            self.payment()
        ...
        while self.arm_state and self.offboard_state and (rospy.is_shutdown() is False):
            ...
            if count > 20:
                ...
                if self.local_pose.pose.position.z < 0.5:
                    print('Mission completed!')
                    print('Pushing to IPFS')
                    client = ipfs.connect()
                    result = client.add('coords_new.txt')
                    hash = result['Hash']
                    robot_key = config.keys()['drone']
                    program = f'echo {hash} | {config.dir()}robonomics io write datalog -s {robot_key}' 
                    process = subprocess.Popen(program, shell=True, stdout=subprocess.PIPE)
                    output = process.stdout.readline()
                    print(f'The IPFS hash is: {output.strip()}')
                    time.sleep(10)
                    exit()
            ...
            f = open('coords_new.txt', 'a')
            f.write(str(self.local_pose.pose.position))
            f.close()
    ...
    def hover(self):
         self.cur_target_pose = self.construct_target(self.local_pose.pose.position.x + 2,
                                                    self.local_pose.pose.position.y + 1,
                                                    self.local_pose.pose.position.z + 3,
                                                    self.current_heading)
